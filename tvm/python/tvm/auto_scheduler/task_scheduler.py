...
class TaskScheduler:
    ...
    def __init__(
        ...
        # <bojian/DietCode> Comment out similarity group, since we are now
        #                   automatically grouping tasks together into a 
        #                   single dynamic search task.
    ...
    def _tune_task(self, task_idx):
        ...
        # )
        num_measure_inputs, best_measure_avg_latency = \
                self.search_policies[task_idx].continue_search_one_round(
                    self.num_measures_per_round,
                    self.measurer
                )
        num_measure_inputs = num_measure_inputs.value
        best_measure_avg_latency = best_measure_avg_latency.value

        # print("num_measure_inputs={}, best_measure_avg_latency={}"
        #       .format(num_measure_inputs, best_measure_avg_latency))
        ...
        if best_measure_avg_latency < self.best_costs[task_idx]:
            self.task_best_cts[task_idx] = self.task_cts[task_idx]
            self.best_costs[task_idx] = best_measure_avg_latency
        ...
        if num_measure_inputs == 0 \
           or no_change_trials > self.early_stopping_task:

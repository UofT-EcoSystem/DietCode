...
class RecordReader(Object):
    ...
    def read_lines(self, max_lines=None, skip_lines=0):
        ...
        # Add a checkpoint here to prevent the compilation from infinitely running.
        # assert False, "Finish loading the records num_inputs={}," \
        #               "num_results={}, num_dispatchers={}" \
        #               .format(len(inputs), len(results), len(dispatchers))
    ...
    def __iter__(self):
        while True:
            ...
            if isinstance(ret, DynWklDispatcher):
                yield ret  # dyn_wkl_dispatcher
            else:
                yield (ret[0], ret[1])  # (input, result)

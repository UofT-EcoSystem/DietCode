...
//     CHECK(!k.empty());
//     size_t ret = hash<T>()(k[0]);
//     for (size_t i = 1; i < k.size(); ++i) {
//       ret = ::dmlc::HashCombine(ret, k[i]);
//     }
//     return ret;
//   }
// };
...
struct TopKDispatcher {
 ...
 public:
  ...
  std::tuple<std::unordered_map<size_t, size_t>,
             ...
             std::vector<float>,
             std::vector<float>>
  MapWklInstsToStates(const std::unordered_map<size_t, size_t>& raw_inst_disp_map,
...
class FlopEstimator : public tir::ExprFunctor<double(const PrimExpr& n)> {
 ...
 public:
  FlopEstimator(const DynShapeVarReplacer& replacer =
                  DynShapeVarReplacer(nullptr))
      : replacer_(replacer) {}
 private:
...
class BlockIdxExtractor : public tir::StmtExprVisitor {
 private:
 ...
 public:
  PrimExpr blockIdx_x_ext;
};

class ThreadIdxExtractor : public tir::StmtExprVisitor {
 private:
  void VisitStmt_(const AttrStmtNode* op) final {
    if (op->attr_key == tir::attr::thread_extent) {
      IterVar iv = Downcast<IterVar>(op->node);

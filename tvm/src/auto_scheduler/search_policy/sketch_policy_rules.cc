...
// <bojian/DietCode>
static std::vector<int> auto_unroll_config_gpu_DietCode = {2048};
...
static std::vector<SplitStepInfo> GetSplitStepsInfoFromWklInst(
    const State* const state, const std::vector<size_t>& split_step_ids,
    const Array<DynShapeVar>& shape_vars, const Array<IntImm>& selected_inst) {
...
static std::vector<SplitStepInfo> GetSplitStepsInfoFromWklInsts(
    const State* const state, const std::vector<size_t>& split_step_ids,
  ...
  for (const size_t split_step_id : split_step_ids) {
    ...
    if (split_step->lengths.size() != 4) {
      ...
      split_steps_info.push_back(
          SplitStepInfo{
            false,
            EvaluateRangeForAllWklInsts(split_step->extent.value(),
                                        shape_vars, wkl_insts).second
          }
          );
    } else {
      split_steps_info.push_back(
  ...
  if (IsDynTask(policy->search_task)) {
    ...
    std::vector<SplitStepInfo> split_steps_info =
        GetSplitStepsInfoFromWklInsts(state, split_step_ids,
                                      policy->search_task->shape_vars.value(),
                                      policy->search_task->wkl_insts);
    if (split_steps_info.empty()) {
      LOG(FATAL) << "split_steps_info is empty in state=" << state->ToStr() << " with "
                    "transform_steps=" << (*state)->transform_steps;
    ...
    try {
      ...
      // if (uniform_real_dist(*rand_gen) < 0.1) {
      //   LOG(INFO) << "Using the pre- hard-coded split factors";
    ...
    //     }
    //     int max_extent =
    //         EvaluateRangeForAllWklInsts(ps->extent.value(),
    //                                     policy->search_task->shape_vars.value(),
    //                                     policy->search_task->wkl_insts).second;
    //     if (enable_verbose_logging) {
    //       LOG(INFO) << "Initializing the tile size for extent=" << ps->extent << " with "
    //                    "max value=" << max_extent;
    //     }
  ...
  *state = policy->search_task->compute_dag.InferBound(*state);
  ...
  for (int stage_id = (*state)->stages.size() - 1; stage_id >= 0; --stage_id) {
    ...
    if (stage->compute_at == ComputeAtKind::kRoot) {
      ...
      if (!multi_level_tiling_root_set.count(stage_id)) {
        ...
        *state = FuseAllOuterSpaceIterators(*state, stage_id, &fused_it);
        ...
        if (GetExtent(fused_it) <= policy->search_task->hardware_params->warp_size) {
          ...
          LOG(FATAL) << "Setting threadIdx.x to the default warp size";
      ...
      int max_total_space_extent =
          EvaluateRangeForAllWklInsts(total_space_extent,
                                      policy->search_task->shape_vars.value(),
                                      policy->search_task->wkl_insts).second;
      if (enable_verbose_logging) {
        LOG(INFO) << "total_space_extent=" << total_space_extent << " w/ max="
                  << max_total_space_extent;
      }

      bool check_min_thread_extent = true;
      // If the total space extent is too small, disable the check of minimal thread extent
      if (max_total_space_extent <= policy->search_task->hardware_params->warp_size * 2) {
      ...
      //   DynShapeVarFinder finder;
      //   finder(blockidx_it->range->extent);
      //   Array<Var> dyn_shape_vars;
      //   for (const DynShapeVarNode* const dyn_shape_var : finder.dyn_shape_vars) {
      //     dyn_shape_vars.push_back(// GetRef<DynShapeVar>(dyn_shape_var)
      //                        Var()
      //                        );
      //   }
...
//   bool opt_tiling_detected = false;
//   {
...
//   [1, 16, ]\n
// ]") {
//       LOG(WARNING) << "Current split_factors=" << OptionalMatrixToString(state->GetSplitFactors());
//       LOG(WARNING) << "split factor captured during evolutionary search";
//       opt_tiling_detected = true;
//       // LOG(WARNING) << "Current max_adapted_score=" << max_adapted_score << ", "
//       //                 "opt_split_factors=" << MatrixToString(opt_split_factors);
//     }
//   }

  // if there are more than one mutated states above the threshold, run the cost
  // model and compare between them
  // if (opt_tiling_detected) {
  //   LOG(INFO) << "The optimal tiling is about to be replaced by "
  //             << MatrixToString(mutated_scheme.split_factors);
  // }

  // static const std::vector<int> shmem_vec_sizes = {4, 3, 2};
  // size_t num_threads_per_block = 1;
  // for (size_t i = 0; i < split_step_ids.size(); ++i) {
  //   const SplitStepNode* const split_step =
  //       (*state)->transform_steps[split_step_ids[i]].as<SplitStepNode>();
  //   if (split_step->lengths.size() == 4) {
  //     num_threads_per_block *= split_step->lengths[1].value()->value;
  //   }
  // }
  ...
  //       continue;
  //     }
  //     CHECK(split_step->lengths.size() == 1);
  //     int local_workspace_size = GetIntImm(split_step->extent.value());
  //     // LOG(INFO) << "local_workspace_size=" << local_workspace_size << ", "
  //     //              "num_threads_per_block=" << num_threads_per_block;
  //     for (const int shmem_vec_size : shmem_vec_sizes) {
  //       if (local_workspace_size %
  //             (shmem_vec_size * num_threads_per_block) == 0) {
  //         // LOG(INFO) << "shmem_vec_size => " << shmem_vec_size;
  //         pstate->transform_steps.Set(
  //             i,
  //             SplitStep(split_step->stage_id, split_step->iter_id,
  //                       split_step->extent, {Integer(shmem_vec_size)},
  //                       split_step->inner_to_outer));
  //         break;
  //       }
  //     }  // for (shmem_vec_size ∈ shmem_vec_sizes)
  //   }    // if (split_step)
  // }      // for (i ∈ (*state)->transform_steps.size())

  // LOG(FATAL) << "Finish a successful mutation";
...
PopulationGenerationRule::ResultKind MutateAutoUnroll::Apply(SketchPolicyNode* policy, State* state,
  ...
  // <bojian/DietCode> Must maintain the maximum unrolling factor
  // int val = auto_unroll_configs[(*rand_gen)() % auto_unroll_configs.size()];
  int val;
  if (IsDynTask(policy->search_task)) {
    val = auto_unroll_config_gpu_DietCode[0];
  } else {
    val = auto_unroll_configs[(*rand_gen)() % auto_unroll_configs.size()];
  }
  StateNode* pstate = state->CopyOnWrite();
  pstate->transform_steps.Set(
      step_id, PragmaStep(ps->stage_id, ps->iter_id,

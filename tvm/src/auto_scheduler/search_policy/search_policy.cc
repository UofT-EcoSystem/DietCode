...
void SearchPolicyNode::PreloadMeasuredStates(const String& log_file) {
  ...
  size_t log_size = // <bojian/DietCode>
                    // res.first.size();
                    std::get<0>(res).size();
  ICHECK_EQ(log_size, // <bojian/DietCode>
                      // res.second.size()
                      std::get<1>(res).size()
            );
  if (log_size) {
    ...
    for (size_t i = 0; i < log_size; i++) {
      ...
      const auto& inp = // <bojian/DietCode>
                        // res.first[i];
                        std::get<0>(res)[i];
      ...
      if (inp->task->workload_key == search_task->workload_key &&
        ...
        measured_throughputs.push_back(
            ...
            // <bojian/DietCode>
            // res.second[i]->error_no == 0 ?
            //   (1.0 / FloatArrayMean(res.second[i]->costs)) : 0.0
            std::get<1>(res)[i]->error_no == 0 ?
              (1.0 / FloatArrayMean(std::get<1>(res)[i]->costs)) : 0.0

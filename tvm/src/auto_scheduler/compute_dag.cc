...
// <bojian/DietCode> FlopEstimator moved to utils.h.
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<SplitStepNode>([](const ObjectRef& ref, ReprPrinter* p) {
      ...
      p->stream << "SplitStepNode("
                     "stage_id=" << node->stage_id << ", "
                     "iter_id=" << node->iter_id << ", "
                     "extent=" << (node->extent == nullptr ?
                                     node->extent :
                                     node->extent.value()) << ", "
...
inline std::string GetIterNamePrefix(const Iterator& iter) {
  ...
  return iter_name.substr(0, pos);
}

std::vector<Iterator> GatherAllItersWithSamePrefix(
    const Array<Iterator>& all_iters, const Iterator& iter_0) {
  std::string iter_0_prefix = GetIterNamePrefix(iter_0);
  std::vector<Iterator> ret_iters;

  for (const Iterator& iter : all_iters) {
    if (iter == iter_0) {
      continue;
    }
    if (GetIterNamePrefix(iter) == iter_0_prefix) {
...
ComputeDAG::InstantiateAndApplySteps(
    const State& state, const Array<DynShapeVar>& shape_vars,
...
ComputeDAG::CherryPickWorkloadInstance(
  ...
  // if (enable_verbose_logging) {
  //   LOG(INFO) << "split_steps="
  //             << OptionalMatrixToString(state_mutable_copy.GetSplitFactors(), true);
  // }
  ...
  for (const Array<IntImm>& wkl_inst : task->wkl_insts) {
    ...
    // adapted_scores.push_back(adapted_score);
    ...
    if (adapted_score == max_adapted_score) {
      ...
      for (size_t i = 0; i < wkl_inst.size(); ++i) {
        if (wkl_inst[i]->value > cherry_picked_wkl_inst[i]->value) {
          if (enable_verbose_logging) {
            LOG(WARNING) << cherry_picked_wkl_inst
                         << " <= " << wkl_inst;
          }
          cherry_picked_wkl_inst = wkl_inst;
          break;
  ...
  // if (enable_verbose_logging) {
  //   LOG(INFO) << "wkl_insts=" << task->wkl_insts << ", "
  //                "adapted_scores=" << ArrayToString(adapted_scores) << ", "
  //                "cherry_picked_shape_values=" << cherry_picked_shape_values;
  // }
...
// class SyntheticWorkloadMutator : public StmtExprMutator {
//  private:
//   HardwareParams hardware_params_;
//  public:
//   SyntheticWorkloadMutator(const HardwareParams& hardware_params)
//       : hardware_params_(hardware_params) {}
//  private:
//   virtual Stmt VisitStmt_(const AttrStmtNode* op) override final {
//     if (op->attr_key == tir::attr::thread_extent) {
//       IterVar iv = Downcast<IterVar>(op->node);
//       if (iv->var->name_hint == "blockIdx.x") {
//         return AttrStmt(op->node, op->attr_key,
//                         Integer(hardware_params_->num_cores) * 2, op->body);
//       }
//     }
//   }
// };
...
ComputeDAG::CherryPickWorkloadInstanceAndApplySteps(
    const State& state, const SearchTask& task) const {
  Array<IntImm> shape_values = std::get<0>(CherryPickWorkloadInstance(state, task));

  if (enable_verbose_logging) {
    LOG(INFO) << "Cherry picked workload inst=" << shape_values;
  }
...
TVM_REGISTER_GLOBAL("auto_scheduler.PrintStateSplitFactors")
    .set_body_typed(
      [](const ComputeDAG& dag, const State& state) -> void {
        ...
        LOG(INFO) << OptionalMatrixToString(state_copy.GetSplitFactors());
      }
    );

    ...
    if (auto* pload = n.as<tir::ProducerLoadNode>()) {
      ...
      if (t->op.defined() && out_dom_map->count(t)) {
        ...
        for (size_t i = 0; i < t.ndim(); ++i) {
          ...
          //              "ConvertedDomMap=" << ConvertDomMap(dom_map);
          // LOG(INFO) << "arg_intset=" << arg_intset;
          ...
          if (arg_interval) {
            ...
            // <bojian/DietCode>
            // LOG(INFO) << "shape_i_min_value=" << shape_i_min_value << ", "
            //              "shape_i_max_value=" << shape_i_max_value;
            // LOG(INFO) << "min_value=" << min_value << ", max_value=" << max_value;
            ...
            //                << analyzer->CanProve(shape_i_max_value <= max_value)
            //           << std::noboolalpha;
            // LOG(INFO) << analyzer->rewrite_simplify(min_value) << " vs. "
            //           << analyzer->rewrite_simplify(shape_i_min_value);
            ...
            if ((arith::is_pos_inf(max_value) && arith::is_neg_inf(min_value)) ||
              ...
              // LOG(WARNING) << "!!! Shrinking the shape size from "
              //                 "[" << shape_i_min_value << ", " << shape_i_max_value << "] to "
              //                 "[" << min_value << ", " << max_value << "] for tensor "
              //              << t->op->name << " !!!";
              // <bojian/DietCode> Reject the shrink to avoid predicates.
              // min_value = shape_i_min_value;
              // max_value = shape_i_max_value;
              if (!(dmlc::GetEnv("DIETCODE_SCHED_OPT", 0) &&
                    t->op->name.find(".shared") != std::string::npos)) {
                min_value = shape_i_min_value;
                max_value = shape_i_max_value;
              // } else {
              //   LOG(WARNING) << "Shrinking is rejected";
              }
            }
...
void BaseComputeOpNode::GatherBound(const Operation& self,
  ...
  for (size_t i = 0; i < this->axis.size(); ++i) {
    ...
    //   LOG(INFO) << axis[i] << " : ∪" << auto_scheduler::ArrayToString(tdom.data.at(i))
    //             << ".cover(" << axis[i]->dom << ")"
    //             << " = " << r;
    // }
  ...
  if (self->reduce_axis.size() != 0) {
    ...
    if (dmlc::GetEnv("DIETCODE_DEBUG_TRACE", 0)) {
      LOG(INFO) << "Making ComputeStmt for " << stage << " with "
                << "main_predicates=" << ArrayToString(n.main_predicates) << " and "
                << "init_predicates=" << ArrayToString(n.init_predicates);
    }
    ...
    //                "post-substitution=" << Substitute(provide, n.main_vmap);
    // }
...
ComputeLoopNest ComputeLoopNest::Create(const BaseComputeOpNode* self, const Stage& stage,
  ...
  // <bojian/DietCode>
  // if (enable_verbose_logging_in_msg_passing) {
  //   LOG(INFO) << "main_predicates.size()=" << ret.main_predicates.size();
  // }
  ...
  if (self->reduce_axis.size() != 0) {
    ...
    // <bojian/DietCode>
    // if (enable_verbose_logging_in_msg_passing) {
    //   LOG(INFO) << "init_predicates.size()=" << ret.init_predicates.size();
    // }

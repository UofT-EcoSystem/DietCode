...
Stmt MakePipeline(const Stage& s, const std::unordered_map<IterVar, Range>& dom_map, Stmt consumer,
                  ...
                  // <bojian/DietCode>
                // , const Array<tir::DynShapeVar>& shape_vars = Array<tir::DynShapeVar>()
                // , const Array<Array<IntImm>>& wkl_insts = Array<Array<IntImm>>()
  ...
  //                "ret=" << s->op->BuildRealize(s, dom_map, pipeline, s->scope);
  // }
...
class InjectAttach : public StmtMutator {
 ...
 private:
  ...
  // <bojian/DietCode>
  // Array<tir::DynShapeVar> shape_vars_;
  // Array<Array<IntImm>> wkl_insts_;
...
Stmt ScheduleOps(Schedule sch, Map<IterVar, Range> dom_map_, bool debug_keep_trivial_loop
  ...
  for (size_t i = sch->stages.size(); i != 0; --i) {
    ...
    // <bojian/DietCode>
    // embed workload instance information into the current stage
    s->shape_vars = shape_vars;
    s->wkl_insts = wkl_insts;

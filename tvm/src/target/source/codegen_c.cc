...
void CodeGenC::AddFunction(const PrimFunc& f) {
  ...
  // <bojian/DietCode>
  // Disable launch_bounds primitives so that we are informed in case the
  // register spilling happens.
  if (dmlc::GetEnv("DIETCODE_ALLOW_REGISTER_SPILL", 0)) {
    LOG(WARNING) << "Register spilling has been enabled";

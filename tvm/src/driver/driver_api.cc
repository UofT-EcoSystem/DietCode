...
namespace tir {
namespace transform {
extern Pass MarkAlreadyOptFlag();
}  // namespace transform
}  // namespace tir
...
// <bojian/DietCode>
using tir::transform::Filter;

transform::Pass CheckOptStatus() {
  return Filter(
           [](const tir::PrimFunc& f) {
             return ((f->GetAttr<Bool>("already_opt", Bool(false))).value()->value);
           }
         );
}

transform::Pass CheckNoOptStatus() {
  return Filter(
           [](const tir::PrimFunc& f) {
             return !(f->GetAttr<Bool>("already_opt", Bool(false))).value()->value;
           }
...
IRModule LowerWithPassList(IRModule mod, Array<tvm::transform::Pass> pass_list) {
  ...
  IRModule opt_mod = CheckOptStatus()(mod);
  mod = optimize(CheckNoOptStatus()(mod));
  mod->Update(opt_mod);
...
IRModule ScheduleToModule(te::Schedule sch, const Array<ObjectRef>& args, const std::string& name,
                          ...
                          // <bojian/DietCode>
                        , const Array<tir::DynShapeVar>& shape_vars
  ...
  if (!shape_vars.empty() && !wkl_insts.empty()) {
    f = WithAttr(std::move(f), "shape_vars", shape_vars);
    f = WithAttr(std::move(f), "wkl_insts", wkl_insts);
    // LOG(INFO) << "Assigning shape_vars=" << shape_vars << ", wkl_insts=" << wkl_insts;
  }
...
TVM_REGISTER_GLOBAL("driver.schedule_to_module")
    .set_body_typed([](te::Schedule sch, const Array<ObjectRef>& args, const String& name,
      ...
      IRModule mod = ScheduleToModule(std::move(sch), args, name, c_binds
                                      ...
                                      // bojian/DietCode
                                    , Array<tir::DynShapeVar>(), Array<Array<IntImm>>()
...
IRModule LowerSchedule(te::Schedule sch, const Array<ObjectRef>& args, const std::string& name,
  ...
  // <bojian/DietCode>
  // LOG(INFO) << "mod=" << mod << ", "
  //              "post-optimization=" << LowerWithPassList(mod, pass_list);
  // LOG(FATAL) << "";
...
std::pair<IRModule, IRModule> SplitDevHostFuncs(IRModule mod_mixed, const Target& target_arg,
  ...
  // <bojian/DietCode>
  // FIXME(bojian) already_opt filtering
  tvm::transform::Pass
      host_filter =
        Filter(
          [](const tir::PrimFunc& f) {
  ...
  IRModule opt_mod_mixed = CheckOptStatus()(mod_mixed);
  ...
  // <bojian/DietCode>
  mixed_pass_list.push_back(tir::transform::MarkAlreadyOptFlag());

  auto opt_mixed = transform::Sequential(mixed_pass_list);
  
  // <bojian/DietCode>
  // mod_mixed = opt_mixed(std::move(mod_mixed));
  mod_mixed = opt_mixed(CheckNoOptStatus()(mod_mixed));
  ...
  // <bojian/DietCode>
  mhost->Update(host_filter(opt_mod_mixed));
  ...
  // <bojian/DietCode>
  mdevice->Update(dev_filter(opt_mod_mixed));
...
runtime::Module build(const Map<Target, IRModule>& inputs_arg, const Target& target_host_arg) {
  ...
  // <bojian/DietCode>
  // for (const std::pair<GlobalVar, BaseFunc>& f : mhost_all->functions) {
  //   std::string func_name(f.first->name_hint);
  //   if (func_name.find("nn_dense") != std::string::npos) {
  //     LOG(INFO) << "mhost.func_name=" << func_name << " : "
  //                  "mhost.func=" << f.second;
  //   }
  //   Optional<String> global_symbol = Downcast<tir::PrimFunc>(f.second)->GetAttr<String>(attr::kGlobalSymbol);
  //   if (global_symbol) {
  //     func_name = global_symbol.value();
  //     if (func_name.find("nn_dense") != std::string::npos) {
  //       LOG(INFO) << "mhost.func_name=" << f.first->name_hint << " : "
  //                    "mhost.func=" << f.second;
  //     }
  //   }
  // }

...
inline IntervalSet Combine<tir::Mul>(Analyzer* analyzer, IntervalSet a, IntervalSet b) {
  ...
  if (b->IsSinglePoint()) {
    ...
    if (tir::canProveForAllDynShapeVars(*analyzer, b->min_value >= 0)) {
...
class IntervalSetEvaluator : public ExprFunctor<IntervalSet(const PrimExpr&)> {
 public:
  ...
  // <bojian/DietCode> Added DynShapeVarNode to the IntervalSet construction.
  IntervalSet VisitExpr_(const DynShapeVarNode* op) final {
    // DynShapeVar dyn_shape_var = GetRef<DynShapeVar>(op);
    // if (op->possible_values.empty()) {
    //   LOG(WARNING) << dyn_shape_var << " does not have possible values specified";
    //   return IntervalSet::SinglePoint(dyn_shape_var);
    ...
    return IntervalSet::SinglePoint(GetRef<DynShapeVar>(op));
  }

...
class TECompilerImpl : public TECompilerNode {
 public:
  ...
  PackedFunc JIT(const CCacheKey& key) final {
    ...
    // <bojian/DietCode>
    if (value->cached_func->funcs->functions.empty()) {
      LOG(FATAL) << "cfunc->ir_module is empty, possible due to the use "
                    "of dynamic modules";
    }
    // LOG(INFO) << "Building for function="
    //                << value->cached_func->prim_fn_var->name_hint << " w/ "
    //              "ir_mod=" << value->cached_func->funcs;
  ...
  Map<String, IRModule> GetLoweredFunctions() {
    ...
    for (const auto& it : cache_) {
      ...
      IRModule ir_mod = lowered_func->cached_func->funcs;
      std::unordered_set<std::string> func_targets_str;
      transform::Pass
          target_filter =
            tir::transform::Filter(
              [&lowered_functions, &func_targets_str](const tir::PrimFunc& f) {
                Optional<Target> func_target = f->GetAttr<Target>("target");
                if (func_target) {

                  // LOG(INFO) << "Filtering " << f->GetAttr<String>(::tvm::attr::kGlobalSymbol).value() << " that has "
                  //              "func_target=" << func_target.value()->str();

                  if (!lowered_functions.count(func_target.value()->str())) {
                    lowered_functions.Set(
                        func_target.value()->str(),
                        IRModule(Map<GlobalVar, BaseFunc>({})));
                  }
                  func_targets_str.insert(func_target.value()->str());
                  return true;
                }
                return false;
              }
      ...
      IRModule ir_mod_w_target_attr = target_filter(ir_mod),
               ir_mod_wo_target_attr = no_target_filter(ir_mod);
      lowered_functions[target->str()]->Update(ir_mod_wo_target_attr);

      // LOG(INFO) << ir_mod_w_target_attr->functions.size() << " and "
      //           << ir_mod_wo_target_attr->functions.size();
      // std::ostringstream strout;
      // strout << "{";
      // for (const std::string& func_target_str : func_targets_str) {
      //   strout << func_target_str << ", ";
      // }
      // strout << "}";
      // LOG(INFO) << "func_targets_str=" << strout.str();

      for (const std::string& func_target_str : func_targets_str) {

        // LOG(INFO) << "func_target_str=" << func_target_str;

        transform::Pass
            detailed_target_filter =
              tir::transform::Filter(
                [&func_target_str](const tir::PrimFunc& f) {
                  Optional<Target> func_target = f->GetAttr<Target>("target");
                  CHECK(func_target);
                  return func_target.value()->str() == func_target_str;
                }
              );
        IRModule ir_mod_w_detailed_target_attr = detailed_target_filter(ir_mod_w_target_attr);
        lowered_functions[func_target_str]->Update(ir_mod_w_detailed_target_attr);
      }
 ...
 private:
  ...
  CCacheValue LowerInternal(const CCacheKey& key, std::function<String(String)> mangle_fn) {
    ...
    te::Schedule sched;
    if (cfunc->sched_or_dyn_wkl_dispatcher->IsInstance<te::ScheduleNode>()) {
      sched = Downcast<te::Schedule>(cfunc->sched_or_dyn_wkl_dispatcher);

      // LOG(INFO) << "Lowering func_name=" << func_name << " w/ "
      //              "all_args=" << auto_scheduler::ArrayToString(all_args) << ", "
      //              "binds=" << auto_scheduler::MapToString(binds) << ", "
      //              "sched=" << tvm::LowerSchedule(sched, all_args, func_name, binds);
      cfunc->funcs->Update(tvm::LowerSchedule(sched, all_args, func_name, binds));
    } else {
      CHECK(cfunc->sched_or_dyn_wkl_dispatcher->
              IsInstance<auto_scheduler::DynWklDispatcherNode>());
      auto_scheduler::DynWklDispatcher dyn_wkl_dispatcher =
          Downcast<auto_scheduler::DynWklDispatcher>(
            cfunc->sched_or_dyn_wkl_dispatcher
          );
      const auto* flower_dyn_wkl_dispatcher =
          runtime::Registry::Get("driver.lower_dyn_wkl_dispatcher");
      ICHECK(flower_dyn_wkl_dispatcher != nullptr)
          << "driver.lower_dyn_wkl_dispatcher is not registered";

      LOG(INFO) << "Code generation for dynamic workload dispatcher";

      ObjectRef ret =
          (*flower_dyn_wkl_dispatcher)(dyn_wkl_dispatcher, cfunc->target,
                                       "llvm", func_name);
      Array<ObjectRef> ir_mod_args_pair = Downcast<Array<ObjectRef>>(ret);
      IRModule ir_mod = Downcast<IRModule>(ir_mod_args_pair[0]);

      LOG(INFO) << "Caching at function " << key->source_func << " w/ "
                   "name=" << func_name;

      // Array<ObjectRef> args = Downcast<Array<ObjectRef>>(ir_mod_args_pair[1]);
      // LOG(FATAL) << "ir_mod=" << ir_mod << ", "
      //               "args=" << auto_scheduler::ArrayToString(args);
      cfunc->funcs->Update(ir_mod);
    }
    value->cached_func = cfunc;
    return value;
  }
 ...
 public:
  ...
  Expr VisitExpr_(const CallNode* call) override {
    ...
    if (func->GetAttr<String>(attr::kCompiler).defined()) {
      ...
      // <bojian/DietCode>
      func_with_metadata = WithAttr(func_with_metadata, "tensor_inputs", ext_func->inputs);
      func_with_metadata = WithAttr(func_with_metadata, "tensor_outputs", ext_func->outputs);
    ...
    // <bojian/DietCode>
    func_with_metadata = WithAttr(func_with_metadata, "tensor_inputs", lowered_func->inputs);
    func_with_metadata = WithAttr(func_with_metadata, "tensor_outputs", lowered_func->outputs);
    ...
    // <bojian/DietCode>
    // LOG(INFO) << "prim_fn_var=" << lowered_func->prim_fn_var;
    // if (lowered_func->prim_fn_var->name_hint == "tvmgen_default_fused_nn_dense") {

    //   LOG(INFO) << "func=" << func;
    //   for (const std::pair<GlobalVar, tir::PrimFunc>& gv_func_pair : prim_fns) {
    //     LOG(INFO) << "\t" << gv_func_pair.first << " : "
    //                       << gv_func_pair.second->GetAttr<String>(::tvm::attr::kGlobalSymbol).value();
    //   }
    // }
...
void UpdateFunctionMetadata(Function relay_func,
  ...
  for (const auto& kv : prim_fns.value()) {
    ...
    // <bojian/DietCode>
    // LOG(INFO) << "prim_fn_target=" << prim_fn_target;
    Optional<Array<te::Tensor>> tensor_inputs =
        relay_func->GetAttr<Array<te::Tensor>>("tensor_inputs");
    Optional<Array<te::Tensor>> tensor_outputs =
        relay_func->GetAttr<Array<te::Tensor>>("tensor_outputs");
    CHECK(bool(tensor_inputs) && bool(tensor_outputs))
        << "Tensor inputs and outputs must be defined";
    // LOG(INFO) << "tensor_inputs="
    //                << auto_scheduler::ArrayToString(tensor_inputs.value()) << ", "
    //              "tensor_outputs="
    //                << auto_scheduler::ArrayToString(tensor_outputs.value());
    
    // determine the IO size from tensors rather than buffer_map
    arith::Analyzer analyzer;
    PrimExpr tensor_io_size = Integer(0);
    for (const te::Tensor& itensor : tensor_inputs.value()) {
      PrimExpr tensor_size;
      for (const PrimExpr& s : itensor->shape) {
        if (!tensor_size.defined()) {
          tensor_size = Integer(1);
        }
        tensor_size *= s;
      }
      if (tensor_size.defined()) {
        tensor_io_size += tensor_size * itensor->dtype.bytes();
      }
    }
    for (const te::Tensor& otensor : tensor_outputs.value()) {
      PrimExpr tensor_size;
      for (const PrimExpr& s : otensor->shape) {
        if (!tensor_size.defined()) {
          tensor_size = Integer(1);
        }
        tensor_size *= s;
      ...
      if (tensor_size.defined()) {
        tensor_io_size += tensor_size * otensor->dtype.bytes();
      }
    }
    tensor_io_size = analyzer.Simplify(tensor_io_size);

    workspace_sizes.Set(prim_fn_target, workspace_size);

    // <bojian/DietCode>
    size_t buffer_io_size = 0;

    // Calculating size for I/O
    for (auto const& param : prim_fn->params) {

      // <bojian/DietCode>
      if (prim_fn->buffer_map.find(param) == prim_fn->buffer_map.end()) {
        // LOG(WARNING) << "param=" << param << " not found in "
        //                   << auto_scheduler::MapToString(prim_fn->buffer_map);
        continue;
      }
      ...
      // <bojian/DietCode>
      buffer_io_size += element_size * num_of_elements;
    }

    // <bojian/DietCode>
    // if (const IntImmNode* const tensor_io_size_val =
    //       tensor_io_size.as<IntImmNode>()) {
    //   if (tensor_io_size_val->value != static_cast<int64_t>(buffer_io_size)) {
    //     LOG(WARNING) << "tensor_io_size=" << tensor_io_size_val->value << " != "
    //                  << buffer_io_size;
    //   }
    // }

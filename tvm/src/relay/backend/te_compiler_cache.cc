...
CachedFunc::CachedFunc(tvm::Target target, GlobalVar prim_fn_var, tvm::Array<te::Tensor> inputs,
  ...
  // <bojian/DietCode>
  // n->schedule = schedule;
  n->sched_or_dyn_wkl_dispatcher = sched_or_dyn_wkl_dispatcher;
...
class ScheduleBuilder : public backend::MemoizedExprTranslator<Array<te::Tensor>> {
 public:
  ...
  CachedFunc Create(const Function& prim_func, std::function<std::string(std::string)> renamer) {
    ...
    // <bojian/DietCode>
    // LOG(INFO) << "Getting the schedule for prim_fn_name=" << prim_fn_name;
    ...
    if (anchor_attrs_.as<DeviceCopyAttrs>() == nullptr) {
      if (use_auto_scheduler_) {
        ...
        sched_or_dyn_wkl_dispatcher =
            (*fauto_schedule)(prim_fn_name, tensor_outs);

        // <bojian/DietCode>
        // LOG(INFO) << "Scheduling result=" << sched_or_dyn_wkl_dispatcher;
        if (!sched_or_dyn_wkl_dispatcher.defined()) {
          LOG(WARNING) << "Unable to obtain schedule for func_name=" << prim_fn_name;
        }
        ...
        if (sched_or_dyn_wkl_dispatcher.defined()) {
          ...
          // if (obj->IsInstance<auto_scheduler::DynWklDispatcherNode>()) {
          if (sched_or_dyn_wkl_dispatcher->
                IsInstance<auto_scheduler::DynWklDispatcherNode>()) {
            auto dyn_wkl_dispatcher =
                Downcast<auto_scheduler::DynWklDispatcher>(
                  sched_or_dyn_wkl_dispatcher
                );
            // LOG(FATAL) << "dyn_wkl_dispatcher=" << dyn_wkl_dispatcher << " "
            //               "obtained while lowering";
          } else {  // if (!obj->IsInstance<auto_scheduler::DynWklDispatcherNode>())
            schedule = Downcast<te::Schedule>(sched_or_dyn_wkl_dispatcher);
          }  // if (!obj->IsInstance<auto_scheduler::DynWklDispatcherNode>())
      ...
      // <bojian/DietCode>
      // if (!schedule.defined()) {
      if (!schedule.defined() && !sched_or_dyn_wkl_dispatcher.defined()) {
    ...
    return CachedFunc(target_, prim_fn_var, fn_inputs, outputs,
                      
                      // <bojian/DietCode>
                      // schedule,
                      schedule.defined() ? schedule : sched_or_dyn_wkl_dispatcher,

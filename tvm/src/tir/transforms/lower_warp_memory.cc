...
Pass LowerWarpMemory() {
  auto pass_func = [](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    // <bojian/DietCode>
    // LOG(INFO) << "f=" << f;
    // auto attr_it = f->attrs->dict.find(tvm::attr::kTarget);
    // LOG(INFO) << "Target=" << ::tvm::auto_scheduler::MapToString(f->attrs->dict);
    // if (attr_it != f->attrs->dict.end()) {
    //   LOG(INFO) << "Target=" << ((*attr_it).second);
    // } else {
    //   LOG(INFO) << "Target attr NOT found";
    // }

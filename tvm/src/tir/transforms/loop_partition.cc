 ...
 public:
  ...
  void VisitStmt_(const AttrStmtNode* op) final {
    if (op->attr_key == attr::thread_extent) {
      ...
      if ((scope.rank == 0) &&  // <bojian/DietCode>
                                // (!is_const_int(op->value) || partition_const_loop_)
          ((!is_const_int(op->value) || partition_const_loop_)
             || dmlc::GetEnv("DIETCODE_SCHED_OPT_PARTITION_BLOCKIDX", 0)
           )
          ) {
      ...
      if (UsesVar(cond, [this](const VarNode* var) { return var == current_var_.get(); })) {
        ...
        if (current_var_->name_hint == "blockIdx.x") {
          ...
          if (const FloorDivNode *floor_div = blockIdx_div_finder.floor_div) {
            ...
            blockIdx_div_max =
                analyzer.Simplify(FloorDiv(hint_map_[current_var_.get()].max(), floor_div->b));
  ...
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    n->body = LoopPartition(std::move(n->body),
                            cfg.value()->partition_const_loop
                              // <bojian/DietCode>
                              || dmlc::GetEnv("DIETCODE_SCHED_OPT_PARTITION_CONST_LOOPS", 0)

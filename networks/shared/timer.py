# timeit_v2.py: Copied from the default library.
# Add 'finish' argument to the default implementation.
...
def py_benchmark(stmt, context, min_repeat_second=1, setup='pass',
                 finish='pass'):
    total_time, number = 0, 10
    eval(stmt, context)  # warmup
    total_time = timeit(stmt=stmt, setup=setup, finish=finish, number=number,
                        globals=context)
    # keep measuring until the total time has reached the min_repeat_second
    while total_time < min_repeat_second:
        number = int(number * (min_repeat_second / total_time)) + 1
        total_time = timeit(stmt=stmt, setup=setup, finish=finish,
                            number=number, globals=context)
